...
//         // let {name, age, status, address} = user;
//         userDiv.innerHTML = `<hr><b>Name - ${user.name}<br>
//         Age - ${user.age}<br>
//         Status - ${user.status}<br></b>`;
//         let addressDiv = document.createElement("div");
//         let {city, country, street, houseNumber} = user.address;
//         addressDiv.innerHTML = `<hr><p>City - ${city}</p>
//                                 <p>Country - ${country}</p>
//                                 <p>Street - ${street}</p>
//                                 <p>House №${houseNumber}</p><hr>`
...
for (let i = 0; i < rules.length; i++) {
    ...
    rulesDiv.classList.add(`rules`, `rule${rule}`);
    let {title, body} = rules[i];
    rulesDiv.innerHTML = `<h2>${title}</h2><p>${body}</p>`
